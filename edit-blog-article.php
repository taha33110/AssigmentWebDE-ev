...
<?php include("head.php");  ?>
    <title>Update Article -  Smarter Blog</title>
...
try {
    ...
    $stmt = $db->prepare('UPDATE blog SET articleTitle = :articleTitle,  articleDescript = :articleDescript, articleContent = :articleContent WHERE articleId = :articleId') ;
$stmt->execute(array(
    ...
    if(isset($error)){
        ...
        try {
           ...
           $stmt = $db->prepare('SELECT articleId,articleTitle, articleDescript, articleContent FROM blog WHERE articleId = :articleId') ;
    ...
    <form action='' method='post'>
       ...
       <h2><label>Short Description(Meta Description) </label><br>
        <textarea name='articleDescript' cols='120' rows='6'><?php echo $row['articleDescript'];?></textarea></h2>
